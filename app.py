...
def main():
    ...
    img = Image.open('used_car.jpg')
    st.image(img, caption= 'Sell Easily!!!', use_column_width=True)
    ...
    years = st.number_input('In which year car was purchased ?',2005, 2020, step=1, key ='year')
    Years_old = 2020-years

    Present_Price = st.slider('What is the current ex-showroom price of the car ?  (In ₹lakhs)', 0.00, 50.00, 5.00, key ='present_price') 
    ...
    Fuel_Type_Petrol = st.radio('What is the fuel type of the car ?',('Petrol','Diesel', 'CNG'), key='fuel')
